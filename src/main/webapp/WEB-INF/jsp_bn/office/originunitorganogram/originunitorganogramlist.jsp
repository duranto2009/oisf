            ...
            <div class="card">
                <div class="portlet box " >
                    <div class="portlet-title">
                        <div class="caption">
                            <i class="fa fa-picture"></i>মৌলিক দপ্তরের পদ ব্যবস্থাপনা
                        </div>
                    </div>
...
<script>
    ...
    var FormValidation = function () {
        var e = function () {
                var e = $("#unit_tree_expand_view #OfficeOriginUnitOrganogramAddForm"),
                    r = $(".alert-danger", e),
                    i = $(".alert-success", e);
                ...
                e.validate({
                    ...
                    focusInvalid: !1, ignore: "", messages: {
                        ...
                        short_name_bng: {
                            required: "বাংলায় মৌলিক দপ্তরের নাম দিন",
                            minlength: "কমপক্ষে ২ টি অক্ষর দিন",
                            maxlength: "১০০ টি অক্ষর এর কম দিন"
                        },
                        short_name_eng: {
                            required: "ইংরেজিতে মৌলিক দপ্তরের নাম দিন",
    ...
    var FormEditValidation = function () {
        var e = function () {
                var e = $("#unit_tree_expand_view #OfficeOriginUnitOrganogramEditForm"),
                    r = $(".alert-danger", e),
                    i = $(".alert-success", e);
    ...
    function addNode() {
        ...
        if($("#unit_tree_expand_view #OfficeOriginUnitOrganogramAddForm").valid()) {
            $.ajax({
                type: "POST",
                ...
                success: function (response) {
                    if (response > 0) {
                        toastr.success("নতুন পদটি সার্থকভাবে সংরক্ষিত হয়েছে");
                        cancel();
                        fetchData($("#origindropdown").val());

                    }
                    else  toastr.error("নতুন পদটি সংরক্ষণ করা সম্ভব হয়নি");
                },
                error: function () {
                    toastr.error("নতুন পদটি সংরক্ষণ করা সম্ভব হয়নি");
                }
            });
        }
    }
    function editNode() {
        ...
        if($("#unit_tree_expand_view #OfficeOriginUnitOrganogramEditForm").valid()) {
            $.ajax({
                type: "POST",
                ...
                success: function (response) {
                    ...
                    if (response > 0) {
                        toastr.success("পদটি সার্থকভাবে সম্পাদন এবং সংরক্ষিত হয়েছে");
                        cancel();
                        fetchData($("#origindropdown").val())

                    }
                    else toastr.error("পদটি সম্পাদন করা সম্ভব হয়নি");
                },
                error: function () {
    ...
    function organogram(parentid, data, id) {
        ...
        for(var i=0;i<childs.length;i++){
            ...
            if(checkChild.length ==0)orgforjstree += '  "text" : "'+childs[i].designationBng+'<a  title=\'এই পদটি বাতিল করুন\' class=\'red equ\'  onclick=showModal(\''+childs[i].id+'\',\'/deleteoriginunitorganogram\')><i class=\'fa fa fa-minus-circle\'></i></a>",';
        ...
        orgforjstree += '{  "text" : "<a  title=\'নতুন পদ যুক্ত করুন\' class=\'red equ\'><i class=\'icon icon-plus\'></i></a>",';
        orgforjstree += '  "icon" : "icon icon-arrow-right",';
    ...
    function organogram(data, id) {
        ...
        for(var i=0;i<data.length;i++){
            ...
            orgforjstree += '  "text" : "'+data[i].designationBng+'<a  title=\'এই পদটি বাতিল করুন\' class=\'red equ\'  onclick=showModal(\''+data[i].id+'\',\'/deleteoriginunitorganogram\')><i class=\'fa fa fa-minus-circle\'></i></a>",';
            orgforjstree += '  "icon" : "icon icon-arrow-right",';
...
//            $('#jstree').jstree('open_node', data.node);
            if(data.node.original.hasOwnProperty("id")){
                if(data.node.original.hasOwnProperty('li_attr')&&data.node.original.li_attr.hasOwnProperty('edit')){
                    $("#unit_tree_expand_view").hide('slow',function () {
                        ...
                        $superiorUnitId.append($('<option></option>').val(0).html('--বাছাই করুন--'));
    ...
    function showModal(id,url) {
        bootbox.confirm({
            message: '<p class="text-center">আপনি কি এই পদটি বাতিল করতে চান?</p>',
            title: "পদটি বাতিল",
            buttons: {
                confirm: {
                    label: 'হ্যাঁ',
                ...
                if (result == true) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            {
                                fetchData($("#origindropdown").val());
                                toastr.success("পদটি সফলভাবে মুছে ফেলা হয়েছে ","সার্থক");

                            }
                            else if(response ==2)
                            {
                                toastr.error(" আপনি যে পদটি মুছতে চাইছেন সেটির আরো শাখা /পদ আছে এই পদটি টি মুছতে হলে এর নিচের সব শাখা/পদ প্রথমে মুছে নিন ","দুঃখিত");
                            }
                            else
                            {
...
//                            {
//
//                                toastr.success("পদটি বাতিল করা হয়েছে");
//                            }
//                            else if(response ==2 )alert("This organogram is used to build real office organogram so you can not delete it . Thank You.");
//                            else toastr.error("পদটি বাতিল করা যায়নি");
                        },
                        error: function () {
                            $('.modal.in .modal-dialog').hide();
                            $(".modal.in .modal-dialog .btn").off("click");
