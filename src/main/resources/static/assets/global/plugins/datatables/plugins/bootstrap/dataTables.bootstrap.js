    ...
    "bootstrap_extended": {
        ...
        "fnUpdate": function (oSettings, fnDraw) {
            ...
            for (i = 0, iLen = an.length; i < iLen; i++) {
                ...
                var totalPages = replaceNumbers("'" + (oPaging.iTotalPages) + "'");
                totalPages = totalPages.replace("'", '');
                totalPages = totalPages.replace("'", '');
                
                var iPages = replaceNumbers("'" + (oPaging.iPage + 1) + "'");
                iPages = iPages.replace("'", '');
                iPages = iPages.replace("'", '');
                
                
                $('.pagination-panel-total', an[i]).html(totalPages);
                $('.pagination-panel-input', an[i]).val(iPages);
