...
public class UpazillaDAO {
    ...
    public ArrayList<UpazillaDTO> getAllUpazilla() throws Exception{
        ...
        try{
            ...
            sql = "select * from geo_upazilas" + " where status=1";
    ...
    public UpazillaDTO getUpazillabyId(int id) throws Exception{
        ...
        try{
            con = DatabaseManager.getInstance().getConnection();
            sql = "select * from geo_upazilas where id = ? and status=1" ;
            ps = con.prepareStatement(sql);
            ps.setInt(1,id);
            ResultSet resultSet = ps.executeQuery();
    ...
    public ArrayList<UpazillaDTO> getUpazillaListbyDistrictId(int districtId) throws Exception{
        ...
        try{
            con = DatabaseManager.getInstance().getConnection();
            sql = "select * from geo_upazilas where geo_district_id = ? and status=1";
            ps = con.prepareStatement(sql);
    ...
    public JSONObject getUpazillaListbyPage(int page, int pageSize) throws Exception{
        Connection con = null;
        PreparedStatement ps = null;
        String sql = null;
        ...
        try{
            con = DatabaseManager.getInstance().getConnection();
            sql = "select * from geo_upazilas where status=1 limit "+(page-1)*pageSize+","+pageSize ;
            ps = con.prepareStatement(sql);
            ResultSet resultSet = ps.executeQuery();
            ...
            int count = sc.tableDatacount("geo_upazilas");
            jsonObject.put("d",upazillaList);
    ...
    public JSONObject getUpazillaListbyPage(Integer pageNumber, int displayLength, String searchparameter) throws Exception{
        Connection con = null;
        ArrayList<UpazillaDTO> upazillaList = new ArrayList<>();
        SQLStatementCreator sc = new SQLStatementCreator();
        JSONObject jsonObject = new JSONObject();
...
//                            " As s");
            ...
            int count = sc.searchCount(con,"geo_upazilas",js);
    ...
    public void add(UpazillaDTO upazilla) throws Exception{
        ...
        try{
            ...
            sql = "insert into geo_upazilas(id, geo_division_id, geo_district_id, division_bbs_code, district_bbs_code, upazila_name_eng, upazila_name_bng, bbs_code, status, created_by, created) values(?,?,?,?,?,?,?,?,?,?,?)";
            ps=con.prepareStatement(sql);
            ...
            ps.setTimestamp(++k, upazilla.getCreated());
    ...
    public void edit(UpazillaDTO upazilla) throws Exception{
        ...
        try {
            ...
            con = DatabaseManager.getInstance().getConnection();
            sql = "Update geo_upazilas SET geo_division_id=?, geo_district_id=?, division_bbs_code=?, district_bbs_code=?, upazila_name_eng=?, upazila_name_bng=?, bbs_code=?, modified_by=?, modified=? WHERE id = ?";
            ps = con.prepareStatement(sql);
            ...
            ps.setTimestamp(++k, upazilla.getModified());
    ...
    //@Razin
    public int delete(UpazillaDTO upazillaDTO) throws Exception{
        String sql = null;
        Connection con = null;
        PreparedStatement ps = null;
        int success = 0;
        try {
            ...
            int k=0;
            ps.setInt(++k,0);
            ps.setInt(++k,upazillaDTO.getModifiedBy());
            ps.setTimestamp(++k,upazillaDTO.getModified());
            ps.setInt(++k,upazillaDTO.getId());
            ps.executeUpdate();
    ...
    public int nextgroup(Connection cn){
        ...
        ResultSet rs=null;
        String sql=" Select group_id from geo_history order by id desc limit 1";
        try {
...
//
    ...
    public boolean changeTotalGeoUpazila
        ...
        try {
            ...
            for (int uniID:sourceUnionListId
                    ) {
                officeDAO.updateOfficeGeo(cn,UPA_ID_COlUMN,parentId,UNI_ID_COlUMN,uniID);

            }
            for (int muniID:sourceMuniListId
                    ) {
                officeDAO.updateOfficeGeo(cn,UPA_ID_COlUMN,parentId,MUNI_ID_COlUMN,muniID);
