...
public class DistrictDAO {
    ...
    private static final String THANA_ID_COlUMN="geo_thana_id";
    private static final String POST_ID_COlUMN="geo_thana_id";
    ...
    public DistrictDTO dtoSetter( ResultSet rs) throws Exception{
        ...
        if(rs.getInt("status")==0)districtDTO.setDistrictNameBng("সাবেক " +rs.getString("district_name_bng"));
        else districtDTO.setDistrictNameBng(rs.getString("district_name_bng"));
    ...
    public DistrictDTO getDistrict(int id) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sql = "select * from " +AllTable.TBL_GEO_DISTRICT+  " where id=?";
        try {
            connection = DatabaseManager.getInstance().getConnection();
            ps = connection.prepareStatement(sql);
            ps.setInt(1, id);
    ...
    public void addDistrict(DistrictDTO districtDTO) throws SQLException {
        ...
        try {
            ...
            ps.setTimestamp(++k, districtDTO.getCreated());
    ...
    public void updateDistrict(DistrictDTO districtDTO) throws Exception {
        ...
        try {
            ...
            ps.setTimestamp(++k, districtDTO.getModified());
    ...
    public int deleteDistrict(DistrictDTO districtDTO)throws Exception {
        ...
        try {
            cn = DatabaseManager.getInstance().getConnection();
            sql = "update " +AllTable.TBL_GEO_DISTRICT+ " set status=0 " + ",modified_by = "+districtDTO.getModifiedBy()+",modified = '"+districtDTO.getModified()+"' where id=? ";
            ps = cn.prepareStatement(sql);
            ps.setInt(1, districtDTO.getId());
            ps.executeUpdate();
            success =1;
        } catch (Exception ex) {
            success =0;
    ...
    public void changeTotalGeoCity
        ...
        String parentName=DIS_ID_COlUMN;
        ...
        try {
            ...
            for (int cityID:sourceListId
                    ) {
                officeDAO.updateOfficeGeo(cn,DIS_ID_COlUMN,parentId,CITY_ID_COlUMN,cityID);
            ...
            ArrayList<Integer>noChilds=new ArrayList<>();
//
            ArrayList<Integer>noChildCity=globalDAO.checkChildCount(cn,
                    sourceDisId,AllTable.TBL_GEO_CITY_CORP,DIS_ID_COlUMN);
            ArrayList<UpazillaDTO>upaCheck=new ArrayList<>();
            ArrayList<ThanaDTO>thaCheck=new ArrayList<>();
            for (int check:noChildCity) {
                upaCheck=upazillaDAO.getUpazillaListbyDistrictId(check) ;
                thaCheck=thanaDAO.getThanaListbyDistrictId(check);
                if(upaCheck.size()==0&&thaCheck.size()==0){
                    noChilds.add(check);
                }
            ...
            globalDAO.updateStatus(cn,noChilds,AllTable.TBL_GEO_DISTRICT);
    ...
    public void changeTotalGeoUpazilla
        ...
        try {
           ...
           // change and deactivate district if it has no child

            ArrayList<Integer>noChilds=new ArrayList<>();
//
           ArrayList<Integer>noChildUpa=globalDAO.checkChildCount(cn,
                   sourceDisId,AllTable.TBL_GEO_UPAZILA,DIS_ID_COlUMN);
           ArrayList<ThanaDTO>thanaCheck=new ArrayList<>();
           ArrayList<CityCorporationDTO>cityCheck=new ArrayList<>();
            for (int check:noChildUpa) {
                thanaCheck=thanaDAO.getThanaListbyDistrictId(check);
                cityCheck=cityCorporationDAO.get(check);
                if(thanaCheck.size()==0&&cityCheck.size()==0){
                    noChilds.add(check);
                }
            ...
            globalDAO.updateStatus(cn,noChilds,AllTable.TBL_GEO_DISTRICT);
//            ArrayList<Integer>noChildThana=globalDAO.checkChildCount(cn,
//                    sourceDisId,AllTable.TBL_GEO_THANA,DIS_ID_COlUMN);
//            ArrayList<Integer>noChildCity=globalDAO.checkChildCount(cn,
//                    sourceDisId,AllTable.TBL_GEO_CITY_CORP,DIS_ID_COlUMN);
//            if(noChildCity.size()==0&&noChildUpa.size()==0&&noChildThana.size()==0){

                //

//            }
//           ArrayList<Integer>thanaListID=new ArrayList<>();
//            for (int noChild :noChilds) {
//
//                ArrayList<ThanaDTO>data=thanaDAO.getThanaListbyDistrictId(noChild);
//                for (ThanaDTO thanaDTO:data) {
//                    thanaListID.add(thanaDTO.getId());
//
//                }
//
//            }
//            changeTotalGeoThana(parentId,userId,thanaListID,sourceDisId, Policy.THANA_TYPE,Policy.THANA_TYPE);
//
//            ArrayList<Integer>cityListID=new ArrayList<>();
//            for (int noChild :noChilds) {
//
//                ArrayList<CityCorporationDTO>data=cityCorporationDAO.get(noChild);
//                for (CityCorporationDTO cityCorporationDTO:data) {
//                    cityListID.add(cityCorporationDTO.getId());
//
//                }
//
//            }
//            changeTotalGeoCity(parentId,userId,cityListID,sourceDisId,Policy.CITY_TYPE,Policy.CITY_TYPE);
//
//            ArrayList<Integer>postListID=new ArrayList<>();
//            for (int noChild :noChilds) {
//
//                ArrayList<PostOfficeDTO>data=postOfficeDAO.postofficebyDis(noChild);
//                for (PostOfficeDTO postOfficeDTO:data) {
//                    postListID.add(postOfficeDTO.getId());
//
//                }
//
//            }
//            changeTotalGeoPost(parentId,userId,postListID,sourceDisId,Policy.POST_OFFICE_TYPE,Policy.POST_OFFICE_TYPE);
//
//            globalDAO.updateStatus(cn,noChilds,AllTable.TBL_GEO_DISTRICT);

            for (int upaID:sourceListId
                    ) {
                officeDAO.updateOfficeGeo(cn,DIS_ID_COlUMN,parentId,UPA_ID_COlUMN,upaID);
    ...
    public void changeTotalGeoThana
        ...
        try {
            ...
            for (int thanaID:sourceListId
                    ) {
                officeDAO.updateOfficeGeo(cn,DIS_ID_COlUMN,parentId,THANA_ID_COlUMN,thanaID);
            ...
            ArrayList<Integer>noChilds=new ArrayList<>();
//
            ArrayList<Integer>noChildTha=globalDAO.checkChildCount(cn,
                    sourceDisId,AllTable.TBL_GEO_THANA,DIS_ID_COlUMN);
            ArrayList<UpazillaDTO>upaCheck=new ArrayList<>();
            ArrayList<CityCorporationDTO>cityCheck=new ArrayList<>();
            for (int check:noChildTha) {
                upaCheck=upazillaDAO.getUpazillaListbyDistrictId(check) ;
                cityCheck=cityCorporationDAO.get(check);
                if(upaCheck.size()==0&&cityCheck.size()==0){
                    noChilds.add(check);
                }
           ...
           // globalDAO.checkChildCount(cn,sourceDisId,AllTable.TBL_,AllTable.TBL_GEO_DISTRICT,SOURCE_COlUMN);
            cn.commit();
    ...
    public void changeTotalGeoPost
        ...
        try {
           ...
           // globalDAO.checkChildCount(cn,sourceDisId,AllTable.TBL_GEO_UPAZILA,AllTable.TBL_GEO_DISTRICT,SOURCE_COlUMN);


            for (int postID:sourceListId
                    ) {
                officeDAO.updateOfficeGeo(cn,DIS_ID_COlUMN,parentId,THANA_ID_COlUMN,postID);
